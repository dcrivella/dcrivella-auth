 ...
 * <p>
 * These properties allow defining audience values per OAuth2 client (RegisteredClient) so that the Authorization Server can mint access
 * tokens targeted to specific Resource Servers. The mapping is read by the token customizer and, when present for a client, is applied to
 * the token's {@code aud} claim as a list of strings.
 * <p>
 * Property prefix: {@code auth.token}
 * <ul>
 * <li>{@code client-audiences} – Map of {@code clientId -> List<String> audiences}. Example:
 *
 * <pre>
 *       auth:
 *         token:
 ...
 * Notes:
 * <ul>
 * <li>Values are opaque to OAuth 2.0; using an {@code api://...} convention is a common practice for readability. Ensure the Resource
 * Server enforces the same audience value(s).</li>
